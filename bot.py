...
def get_countries_content(sid_short):
    ...
    if sid_short in cached_countries_kb:
        return f"🌍 <b>Select a Country to Continue:</b>", cached_countries_kb[sid_short], None
...
def handle_message(message):
    ...
    if text.startswith("/start"):
        ...
        send_message(chat_id, f"🔥 <b>Galaxy BOT</b>\n\nWelcome {html.escape(str(first_name))}, Please Select from menu.", reply_markup=get_main_keyboard(user_id))
...
def handle_callback(callback_query):
    ...
    # ===== User Withdrawal (must be handled BEFORE admin-only callbacks) =====
