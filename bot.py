...
API_2OO9_KEY_DEFAULT = "MUGCQ1WBJQQ"
...
def deliver_to_inbox(user_id, service_name, raw_number, msg_text, current_balance, reward):
    ...
    country_emoji = c_name.split(" ", 1)[0]
    service_name_display = "WhatsApp" if service_name.lower() == "whatsapp" else service_name

    # Admin/inbox notification format. Existing button destinations are preserved.
    text = (
        f"<b>Galaxy BOT</b>                         <b>Admin</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"💬 <b>{html.escape('WS' if service_name.lower() == 'whatsapp' else service_name.upper())}</b> → "
        f"{country_emoji} <code>{html.escape(clean_raw_number[:3] + 'DXA' + clean_raw_number[-3:] if len(clean_raw_number) > 6 else clean_raw_number)}</code>\n"
        f"━━━━━━━━━━━━━━━━━━"
    )

    clean_range = clean_raw_number[:5] if len(clean_raw_number) >= 5 else clean_raw_number
    ...
    row1.append({"text": f"{otp}", "icon_custom_emoji_id": PREMIUM_APPS.get(service_name, PREMIUM_APPS["Other"])["id"], "copy_text": {"text": otp}, "style": "success"})
...
def generate_otp_display(service_name, raw_number, message_text):
    ...
    # Group OTP format. Existing admin-controlled links remain unchanged.
    text = (
        f"<b>Galaxy BOT</b>                         <b>OTP</b>\n\n"
        f"{country_emoji} <b>#{html.escape(c_code.upper())}</b> 🟢 {html.escape(service_name_display)} "
        f"📱+{html.escape(clean_raw_number[:4])}××™{html.escape(clean_raw_number[-4:])} 🤖 <b>#EN</b>\n\n"
        f"                                      🕐 {time_text}\n\n"
        f"🟢 📋 {html.escape(service_name_display)} | {otp}"
    )
    ...
    markup["inline_keyboard"].append(row1)
    markup["inline_keyboard"].append([
        {"text": "NUMBER BOT", "url": deep_link, "icon_custom_emoji_id": "5429167978761461870", "style": "danger"}
    ])
...
def get_live_traffic_content():
    ...
    if not hits_to_show:
        return "╔═════════════════╗\n║  <tg-emoji emoji-id=\"5352877703043258544\">📈</tg-emoji> <b>LIVE TRAFFIC</b>\n╚═════════════════╝\n<i>No traffic data available.</i>\n━━━━━━━━━━━━━━━━━━━", markup
...
def get_services_content():
    ...
    if cached_services_kb:
        return "📱 <b>SELECT APP TO GET SERVICE :</b>", cached_services_kb
...
def get_countries_content(sid_short):
    ...
    if sid_short in cached_countries_kb:
        return f"🌍 <b>Select Country For:</b>", cached_countries_kb[sid_short], None
...
def get_number_allocation_content(sid_short, c_code, user_id=None):
    ...
    if fetched_numbers:
        ...
        msg = (
            f"\n"
            f"<tg-emoji emoji-id=\"5337172996211648018\"></tg-emoji> <b></b>\n"
            f"— — — — — — — — — —\n"
...
def handle_message(message):
    ...
    if text in ["GET NUMBER", "LIVE TRAFFIC", "INVITE FRIEND", "WALLET", "LEADERBOARD", "OWNER PANEL"]:
        if user_id in user_states: del user_states[user_id]
